...
const topics = [
  { id: "technical", label: "Technical issue", description: "Report broken data, wrong scores, UI bugs." },
  { id: "feedback", label: "Feedback", description: "Ideas, feature requests, suggestions." },
  { id: "partnership", label: "Partnership", description: "Business development, cooperation." },
  { id: "advertising", label: "Advertising", description: "Media kit, placements, sponsorships." },
];
  ...
  return (
    <Layout>
      <section className="relative py-10 lg:py-12 px-6 overflow-hidden">
        {/* Background accents */}
        <div className="absolute -top-20 -right-20 w-[400px] h-[400px] bg-powder-sky/10 rounded-full blur-3xl pointer-events-none" aria-hidden />
        <div className="absolute -bottom-32 -left-32 w-[300px] h-[300px] bg-hi-yellow/10 rounded-full blur-3xl pointer-events-none" aria-hidden />
          ...
          <div className="grid lg:grid-cols-[1fr_1fr] gap-8 mb-16">
            <div>
              <div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm border border-frost rounded-pill px-4 py-1.5 mb-6">
                ...
                <span className="font-['DM_Mono'] text-sm text-ink/60">Support</span>
              </div>
            ...
            <div className="flex items-end lg:justify-end">
              <p className="text-lg text-ink/50 max-w-sm leading-relaxed tracking-[-0.01em] lg:text-right">
                Have a question or want to propose a partnership? We typically respond within <strong className="text-ink">1–2 business days</strong>.
              </p>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-[1fr_1fr] gap-6 items-stretch">
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="bg-white border-2 border-ink/[0.06] rounded-[1.25rem] p-7 hover:translate-y-[-2px] hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] transition-all duration-300">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-semibold text-ink tracking-[-0.01em]">Prefer email?</h2>
                    <p className="text-sm text-ink/50 mt-1.5 leading-relaxed">
                      Reach us directly at{" "}
                      <a href="mailto:contact@studevo.com" className="text-ink underline underline-offset-2 hover:text-ink/70 transition-colors">contact@studevo.com</a>
                    </p>
                    <p className="text-xs text-ink/30 mt-2">
                      Tip: include the match link (or match ID) when reporting a technical issue.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border-2 border-ink/[0.06] rounded-[1.25rem] p-7 flex-1">
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    <h2 className="text-lg font-semibold text-ink tracking-[-0.01em]">How can we help?</h2>
                    <p className="text-sm text-ink/50 mt-1.5">Choose a topic to route your request.</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {topics.map((topic) => (
                    ...
                    >
                      <div className={`text-sm font-semibold ${selectedTopic === topic.id ? "text-white" : "text-ink"}`}>{topic.label}</div>
                      <div className={`text-xs mt-1 ${selectedTopic === topic.id ? "text-white/60" : "text-ink/40"}`}>{topic.description}</div>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-white border-2 border-ink/[0.06] rounded-[1.25rem] p-7">
              <div className="flex items-center justify-between mb-1">
                <h2 className="text-lg font-semibold text-ink tracking-[-0.01em]">Send a message</h2>
                <span className="font-['DM_Mono'] text-xs text-ink/40 bg-chalk border border-frost rounded-lg px-3 py-1.5">1–2 days</span>
              </div>
              <p className="text-sm text-ink/40 mb-7">Fill in the form and we'll reply to your email.</p>

              <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block font-['DM_Mono'] text-[11px] text-ink/40 uppercase tracking-widest mb-2">Topic</label>
                    <select
                      value={selectedTopic}
                  ...
                  <div>
                    <label className="block font-['DM_Mono'] text-[11px] text-ink/40 uppercase tracking-widest mb-2">Email</label>
                    <input
                      type="email"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block font-['DM_Mono'] text-[11px] text-ink/40 uppercase tracking-widest mb-2">Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block font-['DM_Mono'] text-[11px] text-ink/40 uppercase tracking-widest mb-2">Subject</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block font-['DM_Mono'] text-[11px] text-ink/40 uppercase tracking-widest mb-2">Message</label>
                  <textarea
                    rows={5}
